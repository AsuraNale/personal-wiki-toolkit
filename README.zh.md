...
你的库可以住在自己电脑上,也可以住在云端 agent session 里。

| | 云端 | 本地 |
|---|---|---|
| 建库、设计结构、写文档 | ✅ 强 | ✅ |
| 问答、策展、随处访问 | ✅ 电脑关机也在跑 | 需要开机 |
| **真的去网上采集** | ⚠️ 受**出口白名单**限制 —— 很多源根本放行不了 | ✅ 没这个限制 |

**第一次建库,建议先从本地开始。** 本地没有出口策略挡路、采集是真能采、一口气就能把闭环跑完。
需要**电脑关机也跑** / **随处访问** / **不想装 Python** 时,再选云端。
**最优解是混合:云端建库 + 策展,本地跑采集。**

云端路径多一个本工具包在本地场景下默认不存在的步骤 —— **配出口白名单** —— 外加几条现实
(库住哪、agent 能推哪些分支)。全部见 [`references/cloud.md`](references/cloud.md)。
...
云端路径 + 其必配的出口白名单步骤,并把 rule 6 扩到覆盖"策略拒绝"(0.1.2)。0.1.1 那三条修补被云端
那次建库**独立复验有效**(它自己读到并全部照做)。**仍 pre-1.0** —— 1.0 打标前继续硬化。方法论提炼自
三个跑了数月、每日自动化 + 独立 QC 的生产资料库。
